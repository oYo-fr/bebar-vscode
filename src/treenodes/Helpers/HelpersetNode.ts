...
export class HelpersetNode extends BaseNode {
  constructor(
    ...
    super(helpersetHandler.helperset.file ?? helpersetHandler.helperset.url!, collapsibleState);
  }

  getChildren(): HelperNode[] {
    let result: HelperNode[] = this.helpersetHandler.helpers.map(p => new HelperNode(this.bebarHandler, p));
